...
CREATE TABLE temp_precinct_tbl (
       election int,
       race int,
       drop_race_name text,
       candidate int,
...
CREATE TABLE temp_demography_precinct (
  precinct int,
  display double precision,
  category text,
  label text,
...
\COPY temp_ward_tbl FROM 'wardALLshortr.csv' DELIMITER ',' CSV HEADER;

\COPY temp_precinct_tbl FROM 'precinctALLshortr.csv' DELIMITER ',' CSV HEADER;

\COPY temp_demography FROM 'CDPDemographyACS2014Ward2015Stacked.csv' DELIMITER ',' CSV HEADER;

\COPY temp_demography_precinct FROM 'CDPDemographyACS2014Precinct2015Stacked.csv' DELIMITER ',' CSV HEADER;
...
CREATE TABLE demography_ward_measures (
...
CREATE TABLE demography_pct_measures (
  measure text PRIMARY KEY,
  category text,
  mean double precision,
  sd double precision
...
CREATE TABLE demography_ward_stdcats (
  measure text REFERENCES demography_ward_measures ON DELETE CASCADE,
  stdcat int,
  stdmin double precision,
  stdmax double precision,
...
CREATE TABLE demography_pct_stdcats (
  measure text REFERENCES demography_pct_measures ON DELETE CASCADE,
  stdcat int,
  stdmin double precision,
  stdmax double precision,
  PRIMARY KEY(measure, stdcat)
...
CREATE TABLE demography_precincts (
  measure text REFERENCES demography_pct_measures ON DELETE CASCADE,
  precinct int,
  display double precision,
  stdcat int,
  std double precision,
  PRIMARY KEY(measure, precinct)
);
...
/* DEMOGRAPHY */
INSERT INTO demography_ward_measures
...
INSERT INTO demography_pct_measures
  SELECT DISTINCT ON (label) label,
    category,
    mean,
    demogsd
  FROM temp_demography_precinct;

INSERT INTO demography_measures
  select measure, category from demography_pct_measures 
  UNION
  select measure, category from demography_ward_measures
  order by measure;


INSERT INTO demography_ward_stdcats
...
INSERT INTO demography_pct_stdcats
  SELECT DISTINCT ON (label, stdcat) label,
    stdcat,
    stdmin,
    stdmax
...
INSERT INTO demography_precincts
  SELECT label,
    precinct,
    display,
    stdcat,
...
CREATE MATERIALIZED VIEW autocompletions AS
  ...
  UNION
  SELECT DISTINCT measure FROM demography_ward_measures;
...
CREATE MATERIALIZED VIEW demography_ward_maps AS
  WITH legends AS (
    ...
    FROM demography_ward_stdcats ws
    JOIN stdcats s ON
      ws.stdcat = s.stdcat
    GROUP BY ws.measure
  ),
...
CREATE MATERIALIZED VIEW demography_precinct_maps AS
  WITH legends AS (
    SELECT ws.measure,
      json_agg(
        json_build_object(
  ...
  ORDER BY ws.stdcat
    ...
    FROM demography_pct_stdcats ws
    JOIN stdcats s ON
      ws.stdcat = s.stdcat
    GROUP BY ws.measure
  ),
