...
const CandidateScatterPlot = ({candidates, zones, onScatterplotChange}) => (
...
export default compose(
	...
	({raceWardMap, raceWardStats}) => ({
	    zones: raceWardStats.map(
		({ward, registeredVoters, turnout}) => [
		    registeredVoters,
		    turnout * 100,
		    raceWardMap[ward] ? raceWardMap[ward] : '#fff'
		]
	    )
	})
    ),
